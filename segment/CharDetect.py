...
def recognizeCharsInPlate(imgThresh, listOfMatchingChars):
    ...
    for currentChar in listOfMatchingChars:                                         # for each char in plate
        ...
        if main.writeresult == True:
            cv2.imwrite(main.resultdst+strCurrentChar+".jpg", crop_img)
    ...
    # end if # show steps #########################################################################
    if main.writeresult == True:
    	cv2.imwrite(main.resultdst+strChars+".jpg",imgThresh)
    return strChars
...
def detectCharsInPlates(possiblePlate):
    intPlateCounter = 0
    imgContours = None
    contours = []
    ...
    cv2.imshow("inputs", possiblePlate.imgPlate)
    #cv2.imshow("treshold", possiblePlate.imgThresh)
